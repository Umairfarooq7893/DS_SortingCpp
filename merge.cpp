...
void merge(int arr[], int low, int mid, int high) {
    ...
    int n1 = mid - low + 1;   // We add +1 because index 0 is also counted.
...
void mergeSort(int arr[], int low, int high) {
    ...
    if (low < high) {
        ...
        mergeSort(arr, low, mid);      // Sort left half recursively untill low < mid
        mergeSort(arr, mid + 1, high); // Sort right half recursively untill low < mid
