...
void countingSort(int arr[], int n) {
    ...
    int index = 0;  // index of actual array
    for(int i = 0; i <= max; i++) {
        while(count[i] > 0) {    //count ma jo index h wahe elements ha
            arr[index] = i;      // place the corresponding occurrences at the corresponding index
